...
PrisonPoly = PolyZone:Create({
    vector2(1500.2009277344, 2444.7353515625),
	vector2(1651.2229003906, 2351.6555175781),
	vector2(1781.3953857422, 2367.322265625),
	vector2(1850.6861572266, 2425.017578125),
	vector2(1889.9183349609, 2488.55859375),
	vector2(1930.8785400391, 2591.4880371094),
	vector2(2016.1322021484, 2663.1154785156),
	vector2(1848.2116699219, 2887.5246582031),
	vector2(1766.0052490234, 2881.119140625),
	vector2(1663.7725830078, 2814.4714355469),
	vector2(1583.4884033203, 2759.33203125),
	vector2(1534.0905761719, 2708.6948242188),
	vector2(1487.5614013672, 2624.6003417969),
	vector2(1473.3303222656, 2545.0617675781)
  }, {
    name = 'prisonpoly',
...
PrisonPoly:onPlayerInOut(function(isPointInside, point)
    ...
    if isPointInside and Config.AntiHelicopter and not (PlayerData.job.name == 'police' or PlayerData.job.name == 'ambulance') then
        triggerAntiHeli()
    elseif not isPointInside and PlayerData.metadata and PlayerData.metadata.injail ~= 0 then
        if Config.JailBreak.JailBreakActive then
            ...
            QBCore.Functions.Notify('The guards caught you escaping..', 'primary', 4000)
...
RegisterNetEvent('qb-jail:client:PrisonServices', function()
    local menu = {
        ...
        },
        {
            ...
            txt = 'Check your remaining time in prison',
...
RegisterNetEvent('qb-jail:client:PrisonRevive', function()
    -- if PlayerData.metadata.injail ~= 0 or PlayerData.job.type == 'leo' then
        ...
        DoScreenFadeOut(1000)
        while not IsScreenFadedOut() do Wait(10) end
        local bedCoords = vector4(1777.6402, 2558.957, 46.722316, 270.17434)
        local ped = PlayerPedId()
...
CreateThread(function()
    ...
    AddTextComponentSubstringPlayerName('Mt Eden Prison')
    EndTextCommandSetBlipName(blip)

    -- Prison Phone
    exports['qb-target']:AddBoxZone('prison_services', vector3(1782.92, 2589.13, 45.8), 0.6, 1.0, {
       ...
       --[[    { -- disabled until lionhearts fixed gks export
                type = 'client',
