...
local function InteractionKeyPressed()
    ...
    elseif LAST_INTERACTION_GAME == "podium" and (IsAtJob(Config.JobName, nil, 1, 3) or PLAYER_IS_ADMIN) then
      FullscreenPrompt(Translation.BOSS_FULLSCREEN_PODIUM_CAPT, Translation.PODIUM_INFO_4, nil, nil)
...
local function InteractableObjectFound(game, entity, coords, model)
    ...
    elseif game == "podium" then
        ...
        if PODIUM_PROPS then
            m = (Translation.PODIUM_INFO):format(PODIUM_PROPS.podiumName)
        else
            m = Translation.PODIUM_INFO_3
        end
        local hasAbility = IsAtJob(Config.JobName, nil, 1, 3) or PLAYER_IS_ADMIN
        if hasAbility then
            m = m .. " " .. Translation.PODIUM_INFO_2
        end
