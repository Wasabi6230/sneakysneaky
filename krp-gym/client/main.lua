...
local function CheckTraining()
	...
	if resting == false then
        QBCore.Functions.Notify('You can now exercise again', 'success')
	end
end
...
function BuyMembership()
	...
	MenuTitle = "Gym Shop"
	ClearMenu()
    Menu.addButton('Buy Membership (Press Enter)', "buyMember",nil)
end
...
CreateThread(function()
	blip = AddBlipForCoord(-1210.465, -1575.804, 4.6079916)
...
-- Mirror
CreateThread(function()
	blip = AddBlipForCoord(813.61, -267.55, 65.89)
	SetBlipSprite(blip, 311)
	SetBlipDisplay(blip, 4)
	SetBlipScale(blip, 0.6)
	SetBlipColour(blip, 7)
...
-- Paleto
CreateThread(function()
	blip = AddBlipForCoord(-304.09, 6118.21, 31.5)
	SetBlipSprite(blip, 311)
	SetBlipDisplay(blip, 4)
	SetBlipScale(blip, 0.6)
	SetBlipColour(blip, 7)
...
local Shops = {
	-- VESP
    [1] = { coords = vector3(-1195.6551, -1577.7689, 4.631155) },
	-- Prison
	[2] = { coords = vector3(1641.39, 2539.09, 45.56) },
	-- Mirror Park
	[3] = { coords = vector3(817.7, -275.45, 66.2) },
	-- Paleto
	[4] = { coords = vector3(-297.52, 6123.29, 31.5) },
}

local checkskills = {
	-- VESP
    [1] = { coords = vector3(-1199.33, -1580.15, 4.61) },
	-- Prison
	[2] = { coords = vector3(1645.51, 2536.87, 45.56) },
	-- Mirror Park
	[3] = { coords = vector3(816.26, -279.06, 66.46) },
	-- Paleto
	[4] = { coords = vector3(-305.1, 6119.02, 31.5) },
}
        ...
        sleep = 1000
            ...
            for k, v in pairs(Shops) do
                ...
                if dist < 4.5 then
                    if dist < 1.5 then
                        sleep = 0
                        DrawText3D(v.coords.x, v.coords.y, v.coords.z, "~b~E~w~ - [Buy Gym Membership]")
                    ...
                    if dist < Config.Locations[k].viewDistance then
                        ...
                        if IsControlJustReleased(0, 38) then
                            if training == false then
                                ...
                                QBCore.Functions.Notify('Starting exercise', 'success')
                                Wait(1000)					
                                if membership == true then
                                    ...
                                    QBCore.Functions.Notify('Nice set! Rest up for 60 seconds', 'error')
                                    training = true
                                    resting = true
                                    CheckTraining()
                                elseif membership == false then
                                    QBCore.Functions.Notify('You need to be a member to work out!', 'error')
                                end
