...
local function saveVehicle()
    ...
    TriggerServerEvent('qb-customswhenua:server:updateVehicle', myCar)
end
...
function AttemptPurchase(type, upgradeLevel)
    ...
    TriggerServerEvent("qb-customswhenua:server:attemptPurchase", type, upgradeLevel, CustomsData.location)
...
function EnterLocation(override)
    ...
    QBCore.Functions.TriggerCallback('qb-vehicletuning:server:IsMechanicAvailable', function(currentMechanics)
        if currentMechanics >= Config.MinOnlineMechanics and not override and PlayerData.job.name ~= 'whenua' then
            repairOnly = true
...
function GetLocations()
    QBCore.Functions.TriggerCallback("qb-customswhenua:server:GetLocations", function(locations)
...
function SetupInteraction()
    QBCore.Functions.TriggerCallback('qb-vehicletuning:server:IsMechanicAvailable', function(currentMechanics)
        ...
        if PlayerData.job.name ~= 'whenua' and Config.DisableWhenMechanicsOnline and currentMechanics >= Config.MinOnlineMechanics then
            text = text .. ' is currently closed'
        else
            if Config.UseRadial then
                if not radialMenuItemId then
                    radialMenuItemId = exports['qb-radialmenu']:AddOption({
                        ...
                        icon = 'wrench',
                        type = 'client',
                        event = 'qb-customswhenua:client:EnterCustoms',
                        shouldClose = true
                    })
                end
            else
...
RegisterNetEvent('qb-customswhenua:client:UpdateLocation', function(location, type, key, value)
    Config.Locations[location][type][key] = value
end)

RegisterNetEvent("qb-customswhenua:client:purchaseSuccessful", function()
    isPurchaseSuccessful = true
    attemptingPurchase = false
    QBCore.Functions.Notify("Purchase Successful")
end)

RegisterNetEvent("qb-customswhenua:client:purchaseFailed", function()
    isPurchaseSuccessful = false
    attemptingPurchase = false
    QBCore.Functions.Notify("Not enough money", "error")
end)

RegisterNetEvent('qb-customswhenua:client:EnterCustoms', function(override)
