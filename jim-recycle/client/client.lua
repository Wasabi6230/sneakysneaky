...
RegisterNetEvent('jim-recycle:SellAnim', function(item)
	-- KRP CUSTOM START
			QBCore.Functions.Progressbar("jim-recycle:SellAnim", "Trading Materials...", 8000, false, false, {
	...
	for _, v in pairs (Peds) do
		if #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(v)) < 3 then
			...
			TaskStartScenarioInPlace(v, "WORLD_HUMAN_STAND_IMPATIENT", -1, true)
...
RegisterNetEvent('jim-recycle:Selling:Menu', function()
	...
	local sellMenu = {
		{ icon = "recyclable_material", header = Loc[Config.Lan].menu["sell_mats"], txt = Loc[Config.Lan].menu["sell_mats_txt"], isMenuHeader = true },
...
RegisterNetEvent('jim-recycle:Trade:Menu', function()
	...
	local tradeMenu = {
		{ icon = "recyclable_material", header = Loc[Config.Lan].menu["mats_trade"], txt = Loc[Config.Lan].menu["trade_amount"]..amount, isMenuHeader = true },
		{ icon = "fas fa-circle-xmark", header = "", txt = Loc[Config.Lan].menu["close"], params = { event = "jim-recycle:CloseMenu" } } }
	if amount >= 1 then tradeMenu[#tradeMenu+1] = { icon = "recyclable_material", header = Loc[Config.Lan].menu["trade1"], params = { event = "jim-recycle:SellAnim", args = 1 } } end
	if amount >= 10 then tradeMenu[#tradeMenu+1] = { icon = "recyclable_material", header = Loc[Config.Lan].menu["trade10"], params = { event = "jim-recycle:SellAnim", args = 2 } } end
	if amount >= 100 then tradeMenu[#tradeMenu+1] = { icon = "recyclable_material", header = Loc[Config.Lan].menu["trade100"], params = { event = "jim-recycle:SellAnim", args = 3 } } end
	if amount >= 500 then tradeMenu[#tradeMenu+1] = { icon = "recyclable_material", header = Loc[Config.Lan].menu["trade500"], params = { event = "jim-recycle:SellAnim", args = 4 } } end
