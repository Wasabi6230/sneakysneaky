...
local function CleanVehicle(veh)
    ...
    QBCore.Functions.Progressbar("cleaning_vehicle", Lang:t("progress.clean_veh"), math.random(10000, 20000), false, true, {
        ...
        -- KRP CUSTOM START (stops hats falling off when using vehicle wash item)
...
-- KRP CUSTOM START (granular repair)
local function RoadsideVehicleFull(veh)
	if (IsBackEngine(GetEntityModel(veh))) then
        SetVehicleDoorOpen(veh, 5, false, false)
    else
        SetVehicleDoorOpen(veh, 4, false, false)
	...
	-- QBCore.Functions.Progressbar("repair_vehicle", Lang:t("progress.roadside_veh"), math.random(15000, 25000), false, true, {
	QBCore.Functions.Progressbar("repair_vehicle", Lang:t("progress.roadside_veh"), math.random(20000, 30000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_player", 1.0)
		QBCore.Functions.Notify(Lang:t("success.repaired_veh_roadside"))
		SetVehicleEngineHealth(veh, Config.RoadsideKitRepair)
		SetVehicleOilLevel(veh, Config.RoadsideKitRepair)
		SetVehicleEngineOn(veh, true, false)
		...
		TriggerServerEvent('qb-vehiclefailure:removeItem', "roadsidekit")
	end, function() -- Cancel
		StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_player", 1.0)
		QBCore.Functions.Notify(Lang:t("error.failed_notification"), "error")
		if (IsBackEngine(GetEntityModel(veh))) then
...
local function PunctureKit(veh)
	...
	QBCore.Functions.Progressbar("repair_vehicle", Lang:t("progress.puncturekit_veh"), math.random(15000, 20000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		QBCore.Functions.Notify(Lang:t("success.repaired_veh_puncturekit"))
...
local function EngineAdditive(veh)
	if (IsBackEngine(GetEntityModel(veh))) then
        SetVehicleDoorOpen(veh, 5, false, false)
    else
        SetVehicleDoorOpen(veh, 4, false, false)
	...
	QBCore.Functions.Progressbar("repair_vehicle", Lang:t("progress.engineadd_veh"), math.random(20000, 25000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_player", 1.0)
		QBCore.Functions.Notify(Lang:t("success.repaired_veh_engineadd"))
		
		SetVehicleEngineHealth(veh, Config.Engineaddvalue)
		SetVehicleOilLevel(veh, Config.Oiladdvalue)
		SetVehicleEngineOn(veh, true, false)
		if (IsBackEngine(GetEntityModel(veh))) then
			SetVehicleDoorShut(veh, 5, false)
		else
		...
		TriggerServerEvent('qb-vehiclefailure:removeItem', "engineadd")
		TriggerServerEvent('hud:server:GainStress', math.random(3, 6))
	end, function() -- Cancel
		StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_player", 1.0)
		QBCore.Functions.Notify(Lang:t("error.failed_notification"), "error")
...
RegisterNetEvent('qb-vehiclefailure:client:RoadsideVehicleFull', function()
	local veh = QBCore.Functions.GetClosestVehicle()
	if veh ~= nil and veh ~= 0 then
		local ped = PlayerPedId()
		local pos = GetEntityCoords(ped)
...
RegisterNetEvent('qb-vehiclefailure:client:PunctureKit', function()
	local veh = QBCore.Functions.GetClosestVehicle()
	if veh ~= nil and veh ~= 0 then
		local ped = PlayerPedId()
		local pos = GetEntityCoords(ped)
...
RegisterNetEvent('qb-vehiclefailure:client:EngineAdditive', function()
	local veh = QBCore.Functions.GetClosestVehicle()
	if veh ~= nil and veh ~= 0 then
		local ped = PlayerPedId()
		local pos = GetEntityCoords(ped)
