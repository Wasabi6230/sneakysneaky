...
function OnDeath()
    if not isDead then
        ...
        -- KRP CUSTOM START (death sound)
        TriggerServerEvent("InteractSound_SV:PlayOnSource", "robloxdeath", 0.1)
        -- KRP CUSTOM END
        ...
        if isDead then
			...
			-- KRP CUSTOM START (Turns off auto alert to EMS when player dies if commented out)
...
CreateThread(function()
	while true do
		...
		if isDead or InLaststand then
			...
			--KRP CUSTOM START (Turn off voice when dead, NOT LAST STAND!)
			...
			-- KRP CUSTOM END
                ...
                if not isInHospitalBed then
                    if deathTime > 0 then
                        ...
                        DrawTxt(0.81, 1.44, 1.0, 1.0, 0.6, Lang:t('info.respawn_revive', {holdtime = hold, cost = Config.BillCost}), 255, 255, 255, 255)
                    end
                end
            ...
            elseif InLaststand then
                ...
                if LaststandTime > Laststand.MinimumRevive then
                    DrawTxt(0.91, 1.44, 1.0, 1.0, 0.6, Lang:t('info.bleed_out', {time = math.ceil(LaststandTime)}), 255, 255, 255, 255)
                else
                    DrawTxt(0.91, 1.44, 1.0, 1.0, 0.6, Lang:t('info.bleed_out_help', {time = math.ceil(LaststandTime)}), 255, 255, 255, 255)
                    if not emsNotified then
                        DrawTxt(0.94, 1.44, 1.0, 1.1, 0.6, Lang:t('info.request_help'), 255, 255, 255, 255)
                    else
                        DrawTxt(0.92, 1.44, 1.0, 1.1, 0.6, Lang:t('info.help_requested'), 255, 255, 255, 255)
                    end
