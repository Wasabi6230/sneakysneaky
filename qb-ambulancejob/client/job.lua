...
function TakeOutVehicle(vehicleInfo)
    ...
    QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
		...
		-- KRP CUSTOM START (Forces St Johns Livery)
...
-- KRP CUSTOM START (fixes animations and adds config for timer)
RegisterNetEvent('hospital:client:RevivePlayer', function()
    ...
    if hasItem then
        ...
        if player ~= -1 and distance < 5.0 then
            local playerId = GetPlayerServerId(player)
            QBCore.Functions.Progressbar("hospital_revive", Lang:t('progress.revive'), Config.ReviveTimer, false, true, {
...
-- (adds a non-emote functions to F1 work menu for EMS as they wanted no emotes for RP reasons)
RegisterNetEvent('hospital:client:RevivePlayerNoEmote', function()
    local hasItem = QBCore.Functions.HasItem('firstaid')
    if hasItem then
        local player, distance = GetClosestPlayer()
        if player ~= -1 and distance < 5.0 then
...
RegisterNetEvent('hospital:client:TreatWounds', function()
    ...
    if hasItem then
        ...
        if player ~= -1 and distance < 5.0 then
            local playerId = GetPlayerServerId(player)
            QBCore.Functions.Progressbar("hospital_healwounds", Lang:t('progress.healing'), Config.TreatWoundTimer, false, true, {
                disableMovement = false,
                disableCarMovement = false,
                disableMouse = false,
                disableCombat = true,
            }, {
                animDict = treatAnimDict,
                anim = treatAnim,
                flags = 33,
            }, {}, {}, function() -- Done
                StopAnimTask(PlayerPedId(), treatAnimDict, "exit", 1.0)
...
-- (adds a non-emote functions to F1 work menu for EMS as they wanted no emotes for RP reasons)
RegisterNetEvent('hospital:client:TreatWoundsNoEmote', function()
    local hasItem = QBCore.Functions.HasItem('bandage')
    if hasItem then
        local player, distance = GetClosestPlayer()
        if player ~= -1 and distance < 5.0 then
