...
-- KRP CUSTOM START
local function VitaminEffects()
    StartScreenEffect("LostTimeNight", 3.0, 0)
    Wait(12000)
	StopScreenEffect("LostTimeNight")
end

RegisterNetEvent('hospital:client:UseVitamins', function()
    local ped = PlayerPedId()
    QBCore.Functions.Progressbar("use_bandage", Lang:t('progress.vitamins'), 12000, false, true, {
        disableMovement = true,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
    }, {
		animDict = "amb@code_human_in_car_mp_actions@first_person@smoke@std@ds@base",
		anim = "enter",
		flags = 16,
    }, {}, {}, function() -- Done
        StopAnimTask(ped, "mp_suicide", "pill", 1.0)
		TriggerServerEvent("hospital:server:removeVitamin")
        TriggerEvent("inventory:client:ItemBox", QBCore.Shared.Items["vitamins"], "remove")
		SetEntityHealth(ped, GetEntityHealth(ped) + 20)
        VitaminEffects()
    end, function() -- Cancel
        StopAnimTask(ped, "amb@code_human_in_car_mp_actions@first_person@smoke@std@ds@base", "enter", 1.0)
        QBCore.Functions.Notify(Lang:t('error.canceled'), "error")
    end)
end)
-- KRP CUSTOM END
