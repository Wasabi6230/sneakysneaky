...
local function open_stash(plate)
     ...
     print(plate)
     TriggerServerEvent("inventory:server:OpenInventory", "stash", "Gunrack-" .. plate, {
          maxweight = Config.gunrack.size,
          slots = Config.gunrack.slots,
     })
     TriggerEvent("inventory:client:SetCurrentStash", "Gunrack-" .. plate)
     if Config.gunrack.while_open_animation then
          Wait(3500)
          repeat
               Wait(1000)
