...
const Open = (data) => {
  ...
  $.each(data.requiredCops, (i, category) => {
	...
	// KRP CUSTOM START
	else if (data.currentEMS >= category.minimumEMS)
	  $(status).html('<i class="fas fa-check"></i>');
	else if (data.currentCorrections >= category.minimumCorrections)
	  $(status).html('<i class="fas fa-check"></i>');
  // KRP CUSTOM END
    else
...
const Setup = (data) => {
  ...
  scoreboardHtml += `
    <div class="scoreboard-info-beam" style="background: #d90d0d">
