...
-- KRP CUSTOM START (MEALS)
RegisterNetEvent('consumables:client:Meal', function(itemName)
    TriggerEvent('animations:client:EmoteCommandStart', {"eat"})
    QBCore.Functions.Progressbar("eat_something", "Eating Meal..", 10000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
        ...
        TriggerEvent("inventory:client:ItemBox", QBCore.Shared.Items[itemName], "remove")
        TriggerEvent('animations:client:EmoteCommandStart', {"c"})
        TriggerServerEvent("consumables:server:addHunger", QBCore.Functions.GetPlayerData().metadata["hunger"] + ConsumablesMeals[itemName])
		TriggerServerEvent("consumables:server:addThirst", QBCore.Functions.GetPlayerData().metadata["thirst"] + ConsumablesMeals[itemName])
        TriggerServerEvent('hud:server:RelieveStress', math.random(10, 20))
    end)
end)

RegisterNetEvent('consumables:client:StressRelief', function()
...
RegisterNetEvent('consumables:client:EcstasyBaggy', function()
    QBCore.Functions.Progressbar("use_ecstasy", "Pops LSD", 3000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
...
RegisterNetEvent('consumables:client:meth', function()
    QBCore.Functions.Progressbar("snort_meth", "Smoking Ass Meth", 5000, false, true, {
...
RegisterNetEvent('consumables:client:UseParachute', function()
    ...
    QBCore.Functions.Progressbar("use_parachute", "Putting on parachute..", 5000, false, true, {
		...
		-- KRP CUSTOM START (diff parachute colours)
		...
		SetPedParachuteTintIndex(ped, Config.ParachuteColour)
		-- KRP CUSTOM END
...
RegisterNetEvent('consumables:client:ResetParachute', function()
    if ParachuteEquiped then
        EquipParachuteAnim()
        QBCore.Functions.Progressbar("reset_parachute", "Packing parachute..", 3000, false, true, {
            disableMovement = false,
            disableCarMovement = false,
            disableMouse = false,
            disableCombat = true,
...
-- KRP CUSTOM START (Light Armor)
RegisterNetEvent('consumables:client:UseLightArmor', function()
    if GetPedArmour(PlayerPedId()) >= 35 then QBCore.Functions.Notify('You already have enough armor on!', 'error') return end
    QBCore.Functions.Progressbar("use_lightarmor", "Putting on the body armour..", 3000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
        ...
        TriggerServerEvent('hospital:server:SetArmor', 35)
        TriggerServerEvent("consumables:server:useLightArmor")
...
RegisterNetEvent('consumables:client:UseArmor', function()
    if GetPedArmour(PlayerPedId()) >= 65 then QBCore.Functions.Notify('You already have enough armor on!', 'error') return end
    QBCore.Functions.Progressbar("use_armor", "Putting on the body armour..", 5000, false, true, {
        ...
        TriggerServerEvent('hospital:server:SetArmor', 65)
