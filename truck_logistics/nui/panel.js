...
window.addEventListener('message', function (event) {
	...
	if (item.notification) {
		...
		if(item.update != true){
			...
			<li onclick="openPage(7)" id="sidebar-drivers-page">
				<i class="fas fa-user-plus"></i>
				<span class="tooltip">${Lang[lang]['sidebar_driver']}</span>
			</li>
			<li onclick="openPage(8)" id="sidebar-my-drivers-page">
			...
			$('#bank-loans-desc').html(`${Lang[lang]['bank_page_loan_desc'].format(currencyFormat(config.max_emprestimo))}`);
			...
			$('#party-form-container-create').append(`
				<input id="party-name" maxlength="30" class="input-party form-control form-control-sm" name="name" type="text" placeholder="${Lang[lang]['party_page_name']}" oninput="InvalidMsg(this);" oninvalid="InvalidMsg(this);" required>
				<input id="party-desc" maxlength="300" class="input-party form-control form-control-sm" name="desc" type="text" placeholder="${Lang[lang]['party_page_subtitle']}" oninput="InvalidMsg(this);" oninvalid="InvalidMsg(this);" required>
				<input id="party-password" maxlength="20" class="input-party form-control form-control-sm input-pass" name="password" type="password" placeholder="${Lang[lang]['party_page_password']}">
				<input id="party-password-confirm" maxlength="20" class="input-party form-control form-control-sm input-pass" name="password-confirm" type="password" placeholder="${Lang[lang]['party_page_password_confirm']}">
				...
				<div class="party-members-container">
					<input id="party-members" max="${config.party.max_members}" style="margin-bottom: 0px;" name="members" class="input-party form-control form-control-sm" type="number" placeholder="${Lang[lang]['party_page_members']}" oninput="InvalidMsg(this,1,${config.party.max_members});" oninvalid="InvalidMsg(this,1,${config.party.max_members});" required>
					<span>${currencyFormat(0)}</span>
				</div>
				<div class="ShowSubmitErrorCreate" style="display:none;"></div>
				<button class="btn btn-primary btn-block submit-party-form btn-sm" id="submit-party-form">${Lang[lang]['party_page_finish_button'].format(currencyFormat(config.party.price_to_create),currencyFormat(0))}</button>
			`);
			...
			$('#party-form-container-join').append(`
				<input id="party-name-join" class="input-party form-control form-control-sm" name="name" type="text" placeholder="${Lang[lang]['party_page_name']}" oninput="InvalidMsg(this);" oninvalid="InvalidMsg(this);" required>
				<input id="party-password-join" class="input-party form-control form-control-sm input-pass" name="password" type="password" placeholder="${Lang[lang]['party_page_password']}">
				<div class="ShowSubmitErrorJoin" style="display:none;"></div>
				<button class="btn btn-primary btn-block submit-party-form btn-sm" id="submit-party-form-join">${Lang[lang]['party_page_finish_button_2']}</button>
			`);
		...
		$('#diagnostic-body').append(`
			<div class="media d-flex">
				<div class="media-body text-left">
					<h3 class="text-success">100 %</h3><span>${Lang[lang]['diagnostic_page_chassi']}</span>
				</div>
		...
		$('#diagnostic-engine').append(`
			<div class="media d-flex">
				<div class="media-body text-left">
					<h3 class="text-success">100 %</h3><span>${Lang[lang]['diagnostic_page_engine']}</span>
				</div>
				<div class="align-self-center">
					<i class="fas fa-tools text-success font-large-2 float-right"></i>
				</div>
		...
		$('#diagnostic-transmission').append(`
			<div class="media d-flex">
				<div class="media-body text-left">
					<h3 class="text-success">100 %</h3><span>${Lang[lang]['diagnostic_page_transmission']}</span>
				</div>
				<div class="align-self-center">
					<i class="fas fa-tools text-success font-large-2 float-right"></i>
				</div>
		...
		$('#diagnostic-wheels').append(`
			<div class="media d-flex">
				<div class="media-body text-left">
					<h3 class="text-success">100 %</h3><span>${Lang[lang]['diagnostic_page_wheels']}</span>
				</div>
				<div class="align-self-center">
					<i class="fas fa-tools text-success font-large-2 float-right"></i>
				</div>
		...
		$('#profile-money').empty();
		$('#profile-money').append(currencyFormat(users.money,0));
		$('#bank-money').empty();
		$('#bank-money').append(currencyFormat(users.money,0));
		
		$('#withdraw-modal-money-available').text(`${Lang[lang]['bank_page_modal_money_available'].format(currencyFormat(users.money))}`);
		$('#deposit-modal-money-available').text(`${Lang[lang]['bank_page_modal_money_available'].format(currencyFormat(item.dados.available_money))}`);
		
		$('#profile-money-earned').empty();
		$('#profile-money-earned').append(currencyFormat(users.total_earned,0));
		$('#profile-deliveries').empty();
		$('#profile-deliveries').append(users.finished_deliveries);
		$('#profile-exp-1').empty();
		$('#profile-exp-1').append(numberFormat(users.exp));
		$('#profile-exp-2').empty();
		var exp_r = 0
		...
		$('#profile-distance-traveled').append(numberFormat(users.traveled_distance,2) + 'km');
				...
				<div class="d-flex flex-row align-items-center"><i class="fas ` + icon + `"></i>
					<div class="ml-2">
						...
						<div class="d-flex flex-row mt-1 text-black-50 date-time">
							<div><i class="fas fa-route"></i></i><span class="ml-2">${Lang[lang]['top_trucker_distance_traveled'].format(numberFormat(top_users.traveled_distance,2))}</span></div>
							<div class="ml-3"><i class="fas fa-chart-line"></i></i><span class="ml-2">${Lang[lang]['top_trucker_exp'].format(numberFormat(top_users.exp))}</span></div>
						</div>
					</div>
				</div>
			</li>`);
		...
		for (const contract of contracts) {
				...
				<li class="d-flex justify-content-between"${border}>
					<div class="d-flex flex-row align-items-center"><i class="${icon}"></i>
						<div class="ml-2">
							...
							<div class="d-flex flex-row mt-1 text-black-50 date-time">
								<div><i class="fas fa-route"></i><span class="ml-2">${Lang[lang]['contract_page_distance'].format(numberFormat(contract.distance,2))}</span></div>
								<div class="ml-3"><i class="fas fa-coins"></i><span class="ml-2">${Lang[lang]['contract_page_reward'].format(currencyFormat(contract.reward))}</span></div>
							</div>
						</div>
					</div>
			...
			if(contract.cargo_type == 1) {
				list_item += '<div data-tooltip="' + Lang[lang]['contract_page_cargo_explosive'] + '"><img src="img/icons/explosive-1.png" width="30"></div>';
			}else if(contract.cargo_type == 2) {
				list_item += '<div data-tooltip="' + Lang[lang]['contract_page_cargo_flammablegas'] + '"><img src="img/icons/flamable-2.png" width="30"></div>';
			}else if(contract.cargo_type == 3) {
				list_item += '<div data-tooltip="' + Lang[lang]['contract_page_cargo_flammableliquid'] + '"><img src="img/icons/flamable-3.png" width="30"></div>';
			}else if(contract.cargo_type == 4) {
				list_item += '<div data-tooltip="' + Lang[lang]['contract_page_cargo_flammablesolid'] + '"><img src="img/icons/flamable-4.png" width="30"></div>';
			}else if(contract.cargo_type == 5) {
				list_item += '<div data-tooltip="' + Lang[lang]['contract_page_cargo_toxic'] + '"><img src="img/icons/toxic-6.png" width="30"></div>';
			}else if(contract.cargo_type == 6) {
				list_item += '<div data-tooltip="' + Lang[lang]['contract_page_cargo_corrosive'] + '"><img src="img/icons/corrosive-8.png" width="30"></div>';
			}
			if(contract.fragile == 1) {
				list_item += '<div data-tooltip="' + Lang[lang]['contract_page_cargo_fragile'] + '"><img src="img/icons/fragile.png" width="30"></div>';
			}
			if(contract.valuable == 1) {
				list_item += '<div data-tooltip="' + Lang[lang]['contract_page_cargo_valuable'] + '"><img src="img/icons/valuable.png" width="30"></div>';
			}
			if(contract.fast == 1) {
				list_item += '<div data-tooltip="' + Lang[lang]['contract_page_cargo_urgent'] + '"><img src="img/icons/fast.png" width="30"></div>';
			}
			var partystart_btn = '';
			if (trucker_party != undefined && !contract.external_data) {
				partystart_btn = `<button onclick="startContract(${contract.contract_id},true)" type="button" class="btn btn-dark waves-effect waves-light">${Lang[lang]['contract_page_button_start_job_party']}</button>`
			}
			let button = `<button onclick="startContract(${contract.contract_id},false)" type="button" class="btn btn-primary waves-effect waves-light">${Lang[lang]['contract_page_button_start_job']}</button>`
			if (contract.progress) {
				button = `<button onclick="cancelContract(${contract.contract_id})" type="button" class="btn btn-outline-danger waves-effect waves-light">${Lang[lang]['contract_page_button_cancel_job']}</button>`
				partystart_btn = '';
			}
			list_item += `
							</div>
					...
					<div class="card-body pt-0 px-0">
						...
						<div class="d-flex flex-row justify-content-between px-3 pb-4">
							...
							<div class="d-flex flex-column">
								<h5 class="mb-0">` + currencyFormat(truck.price) + `</h5>
							</div>
						</div>
						<div class="d-flex flex-row justify-content-between p-3 mid">
							<div class="d-flex flex-column"><small class="text-muted mb-1">${Lang[lang]['dealership_page_engine']}</small>
								<div class="d-flex flex-row"><img src="img/icons/engine_icon.png" style="width:40px;">
									<div class="d-flex flex-column justify-content-center ml-1"><small class="ghj">` + truck.engine + `</small><small class="ghj">` + truck.transmission + `</small></div>
								</div>
							</div>
							<div class="d-flex flex-column"><small class="text-muted mb-2">${Lang[lang]['dealership_page_power']}</small>
								<div class="d-flex flex-row"><img src="img/icons/power_icon.png" style="width:36px;">
									<h6 class="ml-1 mt-1 mb-1 text-muted">${Lang[lang]['dealership_page_power_value'].format(truck.hp)}</h6>
								</div>
							</div>
						</div>
						<div class="mx-3 mt-3 mb-2"><button onclick="buyTruck('` + key + `')" type="button" class="btn btn-primary btn-block"><small>${Lang[lang]['dealership_page_buy_button']}</small></button></div> <small class="d-flex justify-content-center text-muted">${Lang[lang]['dealership_page_bottom_text']}</small>
					</div>
				</div>
				`;
		}
		...
		for (const truck of myTrucks) {
			...
			if(truck.driver == 0){
				...
				$('#diagnostic-body').append(`
					<div class="media d-flex">
						<div class="media-body text-left">
							<h3 class="` + color + `">` + truck.body + ` %</h3><small>${Lang[lang]['diagnostic_page_chassi']} (` + currencyFormat((100-truck.body)*config.repair_price.body) + `)</small>
						</div>
						<div class="align-self-center">
				...
				$('#diagnostic-engine').append(`
					<div class="media d-flex">
						<div class="media-body text-left">
							<h3 class="` + color + `">` + truck.engine + ` %</h3><small>${Lang[lang]['diagnostic_page_engine']} (` + currencyFormat((100-truck.engine)*config.repair_price.engine) + `)</small>
						</div>
						<div class="align-self-center">
							<i class="fas fa-tools ` + color + ` font-large-2 float-right"></i>
						</div>
				...
				$('#diagnostic-transmission').append(`
					<div class="media d-flex">
						<div class="media-body text-left">
							<h3 class="` + color + `">` + truck.transmission + ` %</h3><small>${Lang[lang]['diagnostic_page_transmission']} (` + currencyFormat((100-truck.transmission)*config.repair_price.transmission) + `)</small>
						</div>
						<div class="align-self-center">
							<i class="fas fa-tools ` + color + ` font-large-2 float-right"></i>
						</div>
				...
				$('#diagnostic-wheels').append(`
					<div class="media d-flex">
						<div class="media-body text-left">
							<h3 class="` + color + `">` + truck.wheels + ` %</h3><small>${Lang[lang]['diagnostic_page_wheels']} (` + currencyFormat((100-truck.wheels)*config.repair_price.wheels) + `)</small>
						</div>
						<div class="align-self-center">
							<i class="fas fa-tools ` + color + ` font-large-2 float-right"></i>
						</div>
			...
			list_item += `
				<li class="d-flex justify-content-between">
					...
					<div class="d-flex flex-row align-items-center">
						...
						<button onclick="sellTruck('` + truck.truck_id + `','` + truck.truck_name + `')" class="btn btn-danger">${Lang[lang]['trucks_page_sell_button']}</button>
					</div>
				</li>
				`;
		}
				...
				list_item += `
					<div class="card user-card">
						<div class="card-block">
							...
							<p class="text-muted">${Lang[lang]['drivers_page_hiring_price'].format(currencyFormat(driver.price))}</p>
							<hr>
							<p class="text-muted m-0">${Lang[lang]['drivers_page_product_type']}</p>
							<ul class="list-unstyled activity-leval">
								` + getDriverLevelHTML(driver.product_type) + `
							</ul>
							<p class="text-muted m-0">${Lang[lang]['drivers_page_distance']}</p>
							<ul class="list-unstyled activity-leval">
								` + getDriverLevelHTML(driver.distance) + `
							</ul>
							<p class="text-muted m-0">${Lang[lang]['drivers_page_valuable']}</p>
							<ul class="list-unstyled activity-leval">
								` + getDriverLevelHTML(driver.valuable) + `
							</ul>
							<p class="text-muted m-0">${Lang[lang]['drivers_page_fragile']}</p>
							<ul class="list-unstyled activity-leval">
								` + getDriverLevelHTML(driver.fragile) + `
							</ul>
							<p class="text-muted m-0">${Lang[lang]['drivers_page_urgent']}</p>
							<ul class="list-unstyled activity-leval">
								` + getDriverLevelHTML(driver.fast) + `
							</ul>
							<div onclick="hireDriver('` + driver.driver_id + `')" class="mx-3 mt-3 mb-2"><button type="button" class="btn btn-primary btn-block"><small>${Lang[lang]['drivers_page_hire_button']}</small></button></div>
						</div>
					</div>
					`;
			}else{
					...
					<li class="d-flex justify-content-between">
						<div class="d-flex flex-row align-items-center">
							...
							<div class="ml-2">
								...
								<div class="d-flex flex-row mt-1 text-black-50 date-time">
									<div>
										<i class="fas fa-coins"></i><span class="ml-2">${Lang[lang]['drivers_page_hiring_price'].format(currencyFormat(driver.price))}</span>
									</div>
									<div class="ml-3">
						...
						<div class="d-flex flex-row align-items-center">
							...
							<button onclick="fireDriver('` + driver.driver_id + `')" class="btn btn-danger">${Lang[lang]['drivers_page_fire_button']}</button>
						</div>
					</li>
				`);
			}
		...
		if (trucker_party != undefined) {
			var quit_str = `<button onclick="quitParty()" class="btn btn-danger">${Lang[lang]['party_quit']}</button>`;
			if (trucker_party.owner == 1) {
				quit_str = `<button onclick="deleteParty()" class="btn btn-danger">${Lang[lang]['party_delete']}</button>`;
			}
			$('#party-title-div').empty();
			$('#party-title-div').append(`
			...
			for (const member of trucker_party_members) {
				...
				if (trucker_party.owner == 1 && trucker_party.user_id != member.user_id) {
					kick_str = `<button onclick="kickParty(\'${member.user_id}\')" class="btn btn-danger">${Lang[lang]['party_kick']}</button>`;
				}
				var owner_str = '';
				if (member.owner) {
					owner_str = `<div data-tooltip="${Lang[lang]['party_leader']}"><img src="img/icons/crown.png" width="30"></div>`;
				}
		...
		$(function () {
			...
			$("#party-members").keyup(function () {
				...
				$('.party-members-container span').text(currencyFormat($(this).val() * config.party.price_per_member));
				$('#submit-party-form').text(Lang[lang]['party_page_finish_button'].format(currencyFormat(config.party.price_to_create),currencyFormat($(this).val() * config.party.price_per_member)));
			});
		});
...
function getMyTruckHTML(truck){
	return truck.driver==0 ? `<button onclick="spawnTruck(` + truck.truck_id + `)" class="btn btn-primary mr-2">${Lang[lang]['trucks_page_spawn']}</button> <button onclick="setDriver(null,'` + truck.truck_id + `')" class="btn btn-outline-primary mr-2">${Lang[lang]['trucks_page_remove']}</button>` : `<button onclick="setDriver('0','` + truck.truck_id + `')" class="btn btn-primary mr-2">${Lang[lang]['trucks_page_select']}</button>`
}
...
function startContract(contract_id,party){
	post("startContract",{id:contract_id,party:party})
}
function cancelContract(contract_id){
	post("cancelContract",{id:contract_id})
}
...
function getRoute(name) {
	return "http://truck_logistics/" + name;
}
