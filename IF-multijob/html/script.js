...
function changeButton(id,info){
    if (id === 'duty-button') {
        $(`#${id}`).html(($(`#${id}`).html() === "Select") ? "SELECTED" : "Select") //translatable
        ...
        $('#duty-button').html("Select")//translatable
        $('.job-selectButton').each(function(index) {
            $(this).replaceWith(`<button class="button job-selectButton" id="${$(this).attr('id')}" data-grade="${$(this).data('grade')}">Select</button>`)//translatable
        })
    ...
    window.addEventListener('message', function(event) {
        switch(event.data.action) {
            case "open":
                ...
                `);
                $('#duty-button').html(event.data.duty?"Select":"SELECTED");//translatable
