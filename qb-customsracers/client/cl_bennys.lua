...
local function saveVehicle()
    ...
    TriggerServerEvent('qb-customsracers:server:updateVehicle', myCar)
end
...
function AttemptPurchase(type, upgradeLevel)
    ...
    TriggerServerEvent("qb-customsracers:server:attemptPurchase", type, upgradeLevel, CustomsData.location)
...
function EnterLocation(override)
    ...
    QBCore.Functions.TriggerCallback('qb-vehicletuning:server:IsMechanicAvailable', function(currentMechanics)
        if currentMechanics >= Config.MinOnlineMechanics and not override and PlayerData.job.name ~= 'tuner' then
            repairOnly = true
...
function GetLocations()
    QBCore.Functions.TriggerCallback("qb-customsracers:server:GetLocations", function(locations)
...
function SetupInteraction()
    QBCore.Functions.TriggerCallback('qb-vehicletuning:server:IsMechanicAvailable', function(currentMechanics)
        ...
        if PlayerData.job.name ~= 'tuner' and Config.DisableWhenMechanicsOnline and currentMechanics >= Config.MinOnlineMechanics then
            ...
            if Config.UseRadial then
                if not radialMenuItemId then
                    radialMenuItemId = exports['qb-radialmenu']:AddOption({
                        ...
                        icon = 'wrench',
                        type = 'client',
                        event = 'qb-customsracers:client:EnterCustoms',
                        shouldClose = true
                    })
                end
            else
...
RegisterNetEvent('qb-customsracers:client:UpdateLocation', function(location, type, key, value)
    Config.Locations[location][type][key] = value
end)

RegisterNetEvent("qb-customsracers:client:purchaseSuccessful", function()
    isPurchaseSuccessful = true
    attemptingPurchase = false
    QBCore.Functions.Notify("Purchase Successful")
end)

RegisterNetEvent("qb-customsracers:client:purchaseFailed", function()
    isPurchaseSuccessful = false
    attemptingPurchase = false
    QBCore.Functions.Notify("Not enough money", "error")
end)

RegisterNetEvent('qb-customsracers:client:EnterCustoms', function(override)
