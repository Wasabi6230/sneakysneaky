...
Citizen.CreateThread(function()
    while true do
        ...
        if not JuzBlip then
            ...
            SetBlipColour(Blipy['praca'], 51)
			SetBlipDisplay(Blipy['praca'], 4)
            SetBlipScale(Blipy['praca'], 0.65)
            SetBlipAsShortRange(Blipy['praca'], true)
            BeginTextCommandSetBlipName("STRING")
            AddTextComponentString('Pizza Deliveries')
...
function PullOutVehicle()
    if ownsVan == true then
        QBCore.Functions.Notify("You already have a delivery vehicle, you lost it already!?", "error")
    elseif ownsVan == false then
        coords = Config.Zones.Spawn.Pos
        QBCore.Functions.SpawnVehicle('pizzabike', function(veh)
			SetVehicleNumberPlateText(veh, "PINEAPPL"..tostring(math.random(1000, 9999)))
    ...
    while true do
        ...
        if Hired then
            ...
            if dist <= 2.5 then
                ...
                DrawText3Ds(GaragePos["x"],GaragePos["y"],GaragePos["z"], "Press [~g~G~s~] to start delivering pizzas")
                if dist <= 3.0 then
...
function DrawTarget()
    ...
    if DeliveriesCount == 4 then
      ...
      if pizza == 1 then
        QBCore.Functions.Notify("You have a pizza","success")
      else
        if pizza == 4 then
          pizza = 'Four'
        elseif pizza == 3 then
...
Citizen.CreateThread(function()
    while true do
        ...
        if dist <= 20.0 and Hired and (not HasPizza) then
            ...
            DrawText3Ds(DeliveryPoint["x"],DeliveryPoint["y"],DeliveryPoint["z"], "Grab the ~y~Pizza~s~ from your delivery scooter!")
            local Vehicle = GetClosestVehicle(pos, 6.0, 0, 70)
            if IsVehicleModel(Vehicle, GetHashKey('pizzabike')) then
        ...
        if InService then
            if Delivered then
                if DistanceFromEndZone <= 2.5 then
                    ...
                    DrawText3Ds(endPoint["x"],endPoint["y"],endPoint["z"], "Press [~g~G~s~] to ~r~finish~s~ working")
                    if DistanceFromEndZone <= 7 then
                        if IsControlJustReleased(0, 47) then
                            QBCore.Functions.Notify("Mama Mia! all done?! come back soon ya hear!", "success")
                            EndOfWork()
                        end
                    end
                end
                ...
                if DistanceFromCancelZone <= 2.5 then
                    ...
                    DrawText3Ds(cancel["x"],cancel["y"],cancel["z"], "Press [~g~G~s~] to ~r~cancel~s~ deliveries")
                    if DistanceFromCancelZone <= 7 then
                        if IsControlJustReleased(0, 47) then
                            QBCore.Functions.Notify("Deliveries Canceled!", "error")
							EndOfWork()
                        end
                    end
                end
...
function EndOfWork()
    ...
    if IsPedInAnyVehicle(ped, false) then
        ...
        if IsVehicleModel(Van, GetHashKey('pizzabike')) then
            ...
            QBCore.Functions.Notify("You must return to the delivery scooter!!", "error")
            QBCore.Functions.Notify("If you lost the scooter cancel the job on foot, we ain't paying back your deposit though!", "error")
        end
    else
        InService = false
        BlipSell = nil
