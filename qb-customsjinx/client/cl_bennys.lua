...
local function saveVehicle()
    ...
    TriggerServerEvent('qb-customsjinx:server:updateVehicle', myCar)
end
...
function AttemptPurchase(type, upgradeLevel)
    ...
    TriggerServerEvent("qb-customsjinx:server:attemptPurchase", type, upgradeLevel, CustomsData.location)
...
function EnterLocation(override)
    ...
    QBCore.Functions.TriggerCallback('qb-vehicletuning:server:IsMechanicAvailable', function(currentMechanics)
        if currentMechanics >= Config.MinOnlineMechanics and not override and PlayerData.job.name ~= 'jinx' then
            repairOnly = true
...
function GetLocations()
    QBCore.Functions.TriggerCallback("qb-customsjinx:server:GetLocations", function(locations)
...
function SetupInteraction()
    QBCore.Functions.TriggerCallback('qb-vehicletuning:server:IsMechanicAvailable', function(currentMechanics)
        ...
        if PlayerData.job.name ~= 'jinx' and Config.DisableWhenMechanicsOnline and currentMechanics >= Config.MinOnlineMechanics then
            ...
            if Config.UseRadial then
                if not radialMenuItemId then
                    radialMenuItemId = exports['qb-radialmenu']:AddOption({
                        ...
                        icon = 'wrench',
                        type = 'client',
                        event = 'qb-customsjinx:client:EnterCustoms',
                        shouldClose = true
                    })
                end
            else
...
RegisterNetEvent('qb-customsjinx:client:UpdateLocation', function(location, type, key, value)
    Config.Locations[location][type][key] = value
end)

RegisterNetEvent("qb-customsjinx:client:purchaseSuccessful", function()
    isPurchaseSuccessful = true
    attemptingPurchase = false
    QBCore.Functions.Notify("Purchase Successful")
end)

RegisterNetEvent("qb-customsjinx:client:purchaseFailed", function()
    isPurchaseSuccessful = false
    attemptingPurchase = false
    QBCore.Functions.Notify("Not enough money", "error")
end)

RegisterNetEvent('qb-customsjinx:client:EnterCustoms', function(override)
