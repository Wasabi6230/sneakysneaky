...
RegisterNetEvent('smallresource:client:LoadNitrous', function()
    ...
    if not NitrousActivated then
        if IsInVehicle and not IsThisModelABike(GetEntityModel(GetVehiclePedIsIn(ped))) then
			...
			-- KRP CUSTOM START (increase apply timer)
                QBCore.Functions.Progressbar("use_nos", Lang:t("text.connecting_nos"), 10000, false, true, {
				-- KRP CUSTOM END
            ...
            if VehicleNitrous[Plate] ~= nil then
                if VehicleNitrous[Plate].hasnitro then
                    if IsDisabledControlJustPressed(0, 36) and GetPedInVehicleSeat(CurrentVehicle, -1) == PlayerPedId() then
						...
						-- KRP CUSTOM START
                        SetEntityMaxSpeed(CurrentVehicle, 350.0)
						-- KRP CUSTOM END
