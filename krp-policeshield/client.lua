...
RegisterNetEvent("krpshield:TogglePoliceShield")
AddEventHandler("krpshield:TogglePoliceShield", function()
  Citizen.CreateThread(function()
    if not shield then
      local ped = GetPlayerPed(-1) 
      local propName = "prop_riot_shield"
...
RegisterNetEvent("krpshield:ToggleAOSShield")
AddEventHandler("krpshield:ToggleAOSShield", function()
  Citizen.CreateThread(function()
    if not shield then
      local ped = GetPlayerPed(-1)
      local propName = "prop_ballistic_shield"
      local coords = GetEntityCoords(ped)
      local prop = GetHashKey(propName)
