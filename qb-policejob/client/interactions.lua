...
-- KRP CUSTOM START (Handcuff struggle)
-- This is qb-policejob default
...
-- This is idris one (also see config.lua for max tries)
-- RegisterNetEvent('police:client:GetCuffed', function(playerId, isSoftcuff)
    -- local ped = PlayerPedId()
    -- if gettingCuffed then
        -- return
    -- end
    ...
    -- if not isHandcuffed then
        -- exports['ps-ui']:Circle(function(success)
            -- if not success or escapes >= Config.MaxEscapes then
                -- isHandcuffed = true
                -- TriggerServerEvent("police:server:SetHandcuffStatus", true)
                -- ClearPedTasksImmediately(ped)
                -- if GetSelectedPedWeapon(ped) ~= `WEAPON_UNARMED` then
...
-- This is the qb-locks PR version (Latest)
RegisterNetEvent('police:client:GetCuffed', function(playerId, isSoftcuff)
    if not isHandcuffed then
