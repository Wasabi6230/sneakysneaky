...
function TakeOutVehicle(vehicleInfo)
    ...
    if coords then
        QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
            ...
            SetEntityHeading(veh, coords.w)
			-- KRP CUSTOM START (set vehicle performance)
    ...
    if vehicle ~= 0 and vehicle then
        ...
        if #(pos - vehpos) < 5.0 and not IsPedInAnyVehicle(ped) then
           QBCore.Functions.Progressbar('impound', Lang:t('progressbar.impound'), 5000, false, true, {
                ...
                -- KRP CUSTOM START (Fixes tow/police etc from not being able to /depot locked player cars)
...
-- KRP CUSTOM START (Stops prison guards from accessing evidence)
    if #(pos - takeLoc) <= 1.0 and PlayerJob.grade.level >= 3 then
    ...
    elseif PlayerJob.grade.level <= 3 then
		QBCore.Functions.Notify(Lang:t("error.prison_grade"), "error")
-- KRP CUSTOM END
	else
        exports['qb-menu']:closeMenu()
    end
...
if Config.UseTarget then
    CreateThread(function()
        ...
        for k, v in pairs(Config.Locations["duty"]) do
            exports['qb-target']:AddBoxZone("PoliceDuty_"..k, vector3(v.x, v.y, v.z), 2, 2, {
                name = "PoliceDuty_"..k,
                heading = 11,
                debugPoly = false,
                minZ = v.z - 1,
    ...
    for _, v in pairs(Config.Locations["duty"]) do
        dutyZones[#dutyZones+1] = BoxZone:Create(
            vector3(vector3(v.x, v.y, v.z)), 1.2, 1.2, {
            name="box_zone",
            debugPoly = false,
            minZ = v.z - 1,
            maxZ = v.z + 1,
...
CreateThread(function()
    ...
    for _, v in pairs(Config.Locations["armory"]) do
        armouryZones[#armouryZones+1] = BoxZone:Create(
            vector3(vector3(v.x, v.y, v.z)), 2.5, 2.5, {
            name="box_zone",
            debugPoly = false,
            minZ = v.z - 1,
            maxZ = v.z + 1,
...
function trash()
    CreateThread(function()
        while true do
            Wait(0)
            if inTrash and PlayerJob.name == "police" and PlayerJob.grade.level >= 3 then
			-- if inTrash and PlayerJob.name == "police" then
                if onDuty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
					-- TriggerServerEvent("police:server:clearTrash")
